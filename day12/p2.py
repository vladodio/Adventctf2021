...
counter = 0
def recursive_path_find(currNode, visited, small_cave_visited):
	global counter
	if(currNode.isEnd()):
		counter += 1
		return
	...
	for conn in currNode.getConnections():
		if(conn.getName() == "start"):
			continue
		if(not conn.isSmall()):
			recursive_path_find(conn, visited + " " + currNode.getName(), small_cave_visited)
		elif(not (conn.getName() in visited)):
			recursive_path_find(conn, visited + " " + currNode.getName(), small_cave_visited)
		elif(not small_cave_visited):
			recursive_path_find(conn, visited + " " + currNode.getName(), True)
...
#print(g)
recursive_path_find(g.getStart(), "", False)
print(counter)
